...
window.getIcon = (name, color = null) => {
...
document.addEventListener('click', function (e) {
    ...
    if (e.target.closest('.user-icon') || e.target.closest('.user-link')) {
        ...
        if (tipo == 'usuario') {
            window.location.href = baseurl + `index.php?p=usuarios/get&id=${id}`;
        }
        if (tipo == 'cliente') {
            window.location.href = baseurl + `index.php?p=clientes/get&id=${id}`;
        }
    }
    /*
    ...
    if (e.target.closest('.resultado-item')) {
        ...
        const input = document.querySelector(container?.dataset?.parent);
        const value = e.target.closest('.resultado-item').dataset?.value;

        if (!value || !input || !container) return;
        input.value = value;
    }
