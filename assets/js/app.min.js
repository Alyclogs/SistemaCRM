...
import { verificarEnvios } from "./utils/eventos.js";
...
document.addEventListener('change', function (e) {
    if (e.target.closest('#imageFileInput')) {
        previewImage(e);
    }
});
...
document.addEventListener('click', function (e) {
    ...
    if (e.target.closest('.dropdown-button')) {
        const menu = e.target.closest('.dropdown-button');
        menu.classList.toggle('expanded');
        menu.classList.toggle('selected');
    }
    if (e.target.closest('.list-item')) {
        const container = e.target.closest('.list-item');
        const listView = container.closest(".list-view");
        if (!listView) return;

        const section = listView.querySelector(
            `.section-item[id="${container.dataset.target}"]`
        );
        listView.querySelectorAll(".list-item").forEach(c => c.classList.remove("selected"));
        listView.querySelectorAll(".section-item").forEach(sec => sec.classList.remove("show"));
        container.classList.add("selected");

        if (section) {
            section.classList.add("show");
        }
        const dropdown = container.closest(".list-item-dropdown");
        if (dropdown) {
            dropdown.querySelector(".dropdown-button")?.classList.add("selected");
        }
    }
    /*
    ...
    if (e.target.closest('.edicion-grupo')) {
        ...
        // Generador de inputs según tipo de campo
        const inputHTML = {
            texto: () =>
            ...
            booleano: () =>
                ...
                <option value="1" ${element.textContent.trim() === "Sí" ? "selected" : ""}>Sí</option>
        ...
        // Cancelar → restaurar nodo original
        ...
        // Actualizar → enviar datos y restaurar
        row.querySelector(".btn-actualizar").addEventListener("click", () => {
            ...
            api.post({
                ...
                onSuccess: () => {
                    ...
                    // 👇 Mostrar Sí/No si es booleano
                    if (campo.tipo_dato === "booleano") {
                        original.textContent = newContent === "1" ? "Sí" : "No";
                        ...
                        original.textContent = newContent || "(Vacío)";
                    }
...
document.addEventListener("DOMContentLoaded", function () {
    document.querySelectorAll(".section-item.show").forEach(section => {
        section.querySelector(".section-body").innerHTML = "Cargando...";
    });
    console.log("Ejecutando verificacion de envíos");
    verificarEnvios();
});
